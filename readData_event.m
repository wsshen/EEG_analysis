function readData_event(dataDirPath,eventDirPath)
    
    dirContents = dir([dataDirPath '\*_tsv.txt']);
    eventContents = dir([eventDirPath '\*.txt']);
    bbb= [];
    for i = 1:length(eventContents)
        bbb{end+1} = eventContents(i).name;
    end
