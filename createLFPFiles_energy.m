function createLFPFiles_energy(mice,override)
    LFP_GlobalVariables
    if ~override
...
%                         [f,P1]=readFFT(T(eventInd(k,1):eventInd(k,2)),LFP_data_meanRemoved(eventInd(k,1):eventInd(k,2)));
                        ...
                        [a,b,c,d,e1,e2] = analyzeFrequencyBands(f,10*log10(P1)); %10*log10(P1)
...
%                         totalPower = totalPower + sum(10*log10(P1'));
                        
                    end
                    [P1,f] = readPSD(LFP_data_meanRemoved(:),hann(64),1000);
                    totalPower = mean(10*log10(P1'));
%                     totalPower = totalPower/size(eventInd,1);
                    ...
                    save(saveFileName,'delta','theta','alpha','beta','slow_gamma','fast_gamma','totalPower','f','-mat')
